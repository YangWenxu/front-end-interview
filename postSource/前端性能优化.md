...
### 2. 摇树优化 (Tree-shaking)，
  ...
  通过 ES6 的 `import/export` 来检查未引用代码，以及 `sideEffects` 来标记无副作用代码，最后用 `UglifyJSPlugin` 来做 `Tree Shaking`，从而删除冗余代码。有个问题就是具有副作用的函数无法被 tree-shaking

### 3. code-spliting:代码分割 ，按需加载

 代码分割是指：将脚本中无需立即调用的代码在代码构建时转变为异步加载的过程。

在 Webpack 构建时，会避免加载已声明要异步加载的代码，异步代码在 Webpack 中使用 SplitChunksPlugin 拆分出一个文件，。当代码实际调用时被加载至页面。
...
​	由于 CommonsChunkPlugin 每次构建会重新构建一次 vendor，所以出于效率考虑，使用 `DllPlugin` 将第三方库单独打包到一个文件	中，只有依赖自身发生版本变化时才会重新打包。
